    ...
    static class XaFactoryHelpers
    {
        ...
        public static IDimension ParseDimension(this ParseState state)
        {
            ...
            if (Guid.TryParse(dimensionString, out dimensionId))
            {
                ...
                if (dim == null)
                {
                    throw ParseException.AttributeError(state, string.Format("Dimension '{0}' is not registered", dimensionId));
                }
                return dim;
            }
                ...
                if (!path.StartsWith("/"))
                {
                    var rootItem =
                        ...
                        ExperienceExtractorApiContainer.ItemPaths.GetOrDefault("experienceAnalyticsDimensions") ??
                            "/sitecore/system/Marketing Control Panel/Experience Analytics/Dimensions", state.Parser.DefaultLanguage);
            ...
            throw ParseException.AttributeError(state, string.Format("Dimension item not found \"{0}\"", dimensionString));        
        }        
    }
}
